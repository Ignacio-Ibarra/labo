...
HT.folder <- paste0("HT", dia.mes)

#defino los parametros de la corrida, en una lista, la variable global  PARAM
#  muy pronto esto se leera desde un archivo formato .yaml
PARAM  <- list()

PARAM$experimento  <- HT.folder

PARAM$input$dataset       <- "./exp/FE2809/FE2809_dataset.csv.gz"
# PARAM$input$dataset       <- "./datasets/datasets_muestra25_comp2.csv"
...
EstimarGanancia_lightgbm  <- function( x )
{
  ...
  ganancias <- c()
  for (sema in  primes::generate_n_primes(10)){
  set.seed(sema)
  ...
  ganancia_normalizada  <-  mean(ganancias)* kfolds     #normailizo la ganancia
