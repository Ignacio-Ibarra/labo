...
experimentos <- fread("./exp/HT7231/HT7231.txt")


a <- experimentos[ganancia>= 2.65e+07, ]

#evaluo
bins <- 4
boolean <- quantcut(a$ganancia, q = bins, labels = paste0(c("level"),1:bins)) == paste0("level",bins)
correr <- a[boolean]
...
hs <- makeParamSet( 
  makeNumericParam("learning_rate",    lower = fine.params$learning_rate[1], upper = fine.params$learning_rate[2]),
  makeNumericParam("feature_fraction", lower = fine.params$feature_fraction[1], upper = fine.params$feature_fraction[2]),
  makeIntegerParam("min_data_in_leaf", lower = fine.params$min_data_in_leaf[1], upper = fine.params$min_data_in_leaf[2]),
  makeIntegerParam("num_leaves",       lower = fine.params$num_leaves[1], upper=  fine.params$num_leaves[2]),
  makeIntegerParam("envios",           lower = fine.params$envios[1], upper = fine.params$envios[2]),
  makeInteg
)
...
PARAM$input$dataset       <- "./exp/FE2809/FE2809_dataset.csv.gz"
PARAM$input$training      <- c( 202103 )

PARAM$trainingstrategy$undersampling  <-  1.0   # un undersampling de 0.1  toma solo el 10% de los CONTINUA
PARAM$trainingstrategy$semilla_azar   <- 612337  #Aqui poner la propia semilla
...
PARAM$hyperparametertuning$semilla_azar  <- 612337  #Aqui poner la propia semilla, PUEDE ser distinta a la de trainingstrategy
